...
s.center_frequency = 5e9;                            % 5 GHz carrier frequency
...
l.tx_array = qd_arrayant('testarray');           % Generate Tx antenna
...
%% rx
v =  360/3.6;
t = qd_track('linear',0.1,0);                 % Track of 500 m length, direction SE
t.initial_position = [-250;-250;1.5];                      % Start position
t.interpolate_positions(10000);                           % Interpolate to 5 sample per meter
% t.segment_index = [1,45,97,108,110,160,190,215,235,245,280,295,304,330,400,430 ]; % Segments

Sl = 'WINNER_UMa_C2_LOS';
Sn = 'WINNER_UMa_C4_NLOS1';
% t.scenario = {Sn,Sl,Sn,Sl,Sn,Sn,Sn,Sl,Sn,Sl,Sn,Sl,Sn,Sn,Sn,Sn};
t.set_scenario(Sn,1,0.00001,0.0002,0.0001);
%%       

l.rx_array = qd_arrayant('testarray');           % Rx-Antenna
l.no_rx = Nu; 

for i = 1:Nu
   l.rx_array(1,i).no_elements = 1;
%    l.rx_array(1,i).rotate_pattern(-90,'y');
   l.rx_track(1,i) = copy(t);                                 % Set the rx-track
   l.rx_track(1,i).name = append('UE',num2str(i));
%    l.rx_track(1,i).positions = l.rx_track(1,i).positions + [2;5;0.1].*randn(3,size(l.rx_track(1,i).positions,2));
   l.rx_track(1,i).set_speed(v + (-2+4*rand()));
...
% axis([0 100 min(power)-5 max(power)+5]); grid on;
