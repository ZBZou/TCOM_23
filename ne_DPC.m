function ne_dpc = ne_DPC (Nu, T, s, data, Hst, sigma, M, taps_max,mode)
% Nu = 4;
% T = 100;
% M = 4;
% mode = 1;
...
for i = 1:T
   ...
   P(i) = sum(abs(Tx_signal(:,i)).^2)/Nu; %% power
   Tx_signal(:,i) = Tx_signal(:,i)/sqrt(P(i));%power constraint
end
...
%% estimation
for i = 1:T
   Rx_signal(:,i) = Rx_signal(:,i) * sqrt(P(i)); % normalize
   inv(diag(diag(L(:,:,i))));
